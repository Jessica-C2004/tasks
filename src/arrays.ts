...
export function stringsToIntegers(numbers: string[]): number[] {
    const numArr: number[] = numbers.map((str: string): number =>
...
export function injectPositive(values: number[]): number[] {
    ...
    const negIndex = newValues.findIndex((value: number): boolean => value < 0);
    if (negIndex === -1) {
        ...
        const beforeNegNum = newValues
            .slice(0, negIndex)
            .reduce((currTotal: number, num: number) => currTotal + num, 0);
        newValues.splice(negIndex + 1, 0, beforeNegNum);
    }
